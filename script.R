#### Formation bonnes pratiques R - Script sale a corriger
...
### Selection d'un sous ensemble de variables
df2 <- df %>%
...
ggplot(df2[as.numeric(df2$aged)>50,c(3,4)], aes(x=as.numeric(aged),#x = as.numeric(aged) - as.numeric(aged) %% 5,
                                                y = ..density..,
...
library(forcats)
df2$sexe <- fct_recode(as.character(df2$sexe),
                       "Homme"="0",
                       "Femme"="1")
